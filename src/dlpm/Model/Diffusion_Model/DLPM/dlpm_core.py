...
class DLPM:
    ...
    def _clip_sample_xstart(self, xstart):
        clip = getattr(self, 'sample_x0_clip', None)
        if clip is None:
            return xstart
        clip = float(clip)
        if clip <= 0:
            return xstart
        return torch.clamp(xstart, -clip, clip)
    ...
    def anterior_mean_variance_dlim(self, x_t, t, eps, t_prev=None, eta=0.0):
        ...
        x0_hat = self._clip_sample_xstart((x_t - bs * eps) / bg.clamp(min=1e-12))

        if eta == 0.0:
            sample = bg_p * x0_hat + bs_p * eps
        ...
        mean = bg_p * x0_hat + diff_term * eps
    ...
    def anterior_mean_variance_dlpm(self, x_t, t, eps, t_prev=None):
        """Compute the DLPM posterior for adjacent or skipped transitions."""
        # 确保t是批次张量
        t_batch = self.get_t_to_batch_size(x_t, t)
        ...
        if t_prev is None:
            ...
            tp_val = int(t_prev.reshape(-1)[0].item())
        else:
            tp_val = int(t_prev)
        tp_val = max(0, min(tp_val, t_val - 1))
        ...
        Sigma_t_1_val = self.Sigmas[tp_val]
        ...
        _, bg, _, bs = self.get_schedule_at_t(t, x_t.shape)
        _, bg_prev, _, _ = self.get_schedule_at_t(tp_val, x_t.shape)
        transition_gamma = (bg / bg_prev.clamp(min=1e-12)).clamp(min=1e-12)
        Gamma_t = 1 - (transition_gamma ** 2 * Sigma_t_1_val) / Sigma_t_val.clamp(min=1e-12)
        Gamma_t = torch.clamp(Gamma_t, min=0.0, max=1.0)
        x_t_1 = (x_t - bs * Gamma_t * eps) / transition_gamma

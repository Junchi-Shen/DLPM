...
class DataProcessor:
    ...
    def _encode_ids(self, df, col, default_name="__default__", table=None):
        """Encode categories, reusing the training table during evaluation.

        Rebuilding a category table on a test subset silently changes index IDs
        in multi-index experiments. Unknown evaluation categories use a
        reserved embedding slot instead of changing known IDs.
        """
        ...
        unknown_id = len(table)
        ids = col_s.map(table).fillna(unknown_id).astype(int)
        return ids, table
    ...
    def _parse_numeric_vector(self, value, expected_len=None):
        if isinstance(value, np.ndarray):
            ...
            # CSV readers return scalar history summaries as numpy.float64.
    ...
    def _history_feature_columns(self, df):
        cols = self.config.get('history_feature_columns', [])
        if cols:
            missing = [c for c in cols if c not in df.columns]
            if missing:
                raise ValueError(
                    'Configured history features are missing from the dataset: '
                    + ', '.join(missing)
                )
            return list(cols)
        if not self.config.get('use_history_context', False):
            return []
        default_cols = [
        ...
        return [c for c in default_cols if c in df.columns]

    def _build_history_features(self, df):
        cols = self._history_feature_columns(df)
        if not cols:
            self.history_feature_columns = []
            self.history_feature_dim = 0
            return None
        fixed_lengths = self.config.get('history_feature_lengths', {
            'hist_returns_60': 60,
        ...
        offset = 0
        for col in cols:
            expected_len = fixed_lengths.get(col)
            parsed = df[col].apply(lambda x: self._parse_numeric_vector(x, expected_len=expected_len))
            block = np.vstack(parsed.values).astype(np.float32)
            if block.ndim == 1:
                block = block.reshape(-1, 1)
            blocks.append(block)
            width = block.shape[1]
            feature_slices[col] = (offset, width)
            offset += width
            expanded_names.extend([col] if width == 1 else [f'{col}_{i}' for i in range(width)])
        extra = np.concatenate(blocks, axis=1).astype(np.float32)
        self.history_feature_columns = expanded_names
        self.history_feature_dim = int(extra.shape[1])
        self.history_feature_slices = feature_slices
        logging.info(f"Loaded history context features: {self.history_feature_dim} dims")
        return extra
    ...
    def create_condition_tensors(self, df, fit_scaler=False):
        ...
        try:
            # === 1. 数值特征（需要标准化）===
            price_transform = str(self.config.get('condition_price_transform', 'linear_scaled')).lower()
            raw_prices = np.maximum(df['S_0'].astype(float).values, 1e-12)
            prices = (np.log(raw_prices) if price_transform == 'log'
                      else raw_prices / self.config['base_trading_days'])
            ...
            history_features = self._build_history_features(df)
            if history_features is not None:
                numerical_features = np.column_stack([numerical_features, history_features]).astype(np.float32)
            # Keep archived 377-feature runs readable after fixing scalar
            # parsing. New runs fit a 389-feature scaler; old runs retain
            # their original array-only condition layout for reproducibility.
            expected_features = getattr(self.price_scaler, 'n_features', None)
            if expected_features is None:
                expected_features = getattr(getattr(self.price_scaler, 'robust_scaler', None), 'n_features_in_', None)
            if not fit_scaler and expected_features is not None and numerical_features.shape[1] != int(expected_features):
                expected_features = int(expected_features)
                if numerical_features.shape[1] > expected_features:
                    numerical_features = numerical_features[:, :expected_features]
                else:
                    pad = np.zeros((len(numerical_features), expected_features - numerical_features.shape[1]), dtype=np.float32)
                    numerical_features = np.column_stack([numerical_features, pad])
            self.numerical_feature_dim = int(numerical_features.shape[1])
            
            # === 2. 类别特征（不标准化）===
            if 'country' in df.columns:
                if fit_scaler or self.country_table is None:
                    country_id, self.country_table = self._encode_ids(df, 'country')
                else:
                    country_id, _ = self._encode_ids(df, 'country', table=self.country_table)
            ...
            if 'asset_underlying' in df.columns:
                if fit_scaler or self.index_table is None:
                    index_id, self.index_table = self._encode_ids(df, 'asset_underlying')
                else:
                    index_id, _ = self._encode_ids(df, 'asset_underlying', table=self.index_table)
            ...
            return {
                ...
                'history_feature_dim': getattr(self, 'history_feature_dim', 0)
            }
    ...
    def recover_from_prediction(self, x_sample, y_pred):
        ...
        try:
            # 只取前5个数值特征进行逆变换
            n_features = getattr(self.price_scaler, 'n_features', 5) or 5
            x_numerical = np.zeros((1, n_features), dtype=np.float32)
            x_numerical[0, :5] = x_sample[:5]
            unscaled_features = self.price_scaler.inverse_transform(x_numerical)
            
            normalized_price = unscaled_features[0, 0]
            if str(self.config.get('condition_price_transform', 'linear_scaled')).lower() == 'log':
                start_price = float(np.exp(normalized_price))
            else:
    ...
    def process_all_data(self, path, fit_scaler=True):
        """一键处理数据。

        ``fit_scaler=True`` is used for the training split. Validation and
        test splits must pass ``False`` so that their numerical features and
        categorical IDs stay in the training coordinate system.
        """
        try:
            print("=" * 60)
            print("开始数据处理流程...")
            ...
            print("\n4. 创建条件张量...")
            condition_dict = self.create_condition_tensors(
                df_transformed, fit_scaler=fit_scaler
            )

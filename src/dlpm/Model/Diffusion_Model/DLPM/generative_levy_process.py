...
class GenerativeLevyProcess:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=False, denoised_fn=None, model_kwargs=None, eps_only=False):
        ...
        def process_xstart(x):
            if denoised_fn is not None:
                x = denoised_fn(x)
            x = self.dlpm._clip_sample_xstart(x)
            if clip_denoised:
                return x.clamp(-1, 1)
            return x
        ...
        if self.model_mean_type == ModelMeanType.EPSILON:
            # Apply the configured financial x0 bound to ancestral sampling.
            # The old branch skipped this whenever clip_denoised=False, which
            # made sample_x0_clip ineffective and allowed the final token to
            # grow without a model-scale bound.
            if clip_denoised or self.dlpm.sample_x0_clip is not None:
    ...
    def native_skip_sample(self, model, x, t, t_prev, clip_denoised=False,
                           denoised_fn=None, model_kwargs=None):
        """A stochastic skip transition using the DLPM cumulative posterior.

        Unlike the DDIM-style path, this branch keeps the sampled auxiliary
        scale sequence in both the skipped posterior mean and variance.
        """
        out = self.p_mean_variance(
            model, x, t,
            clip_denoised=clip_denoised,
        ...
        eps = out['eps']
        mean, variance = self.dlpm.anterior_mean_variance_dlpm(
            x, t, eps, t_prev=t_prev)
        noise = torch.randn_like(x)
        nonzero_mask = ((t != 1).float().view(-1, *([1] * (len(x.shape) - 1))))
        sample = mean + nonzero_mask * torch.sqrt(torch.clamp(variance, min=0.0)) * noise
        return {"sample": sample}

    def native_skip_sample_loop_progressive(self, model, shape, noise=None,
                                            clip_denoised=False,
                                            denoised_fn=None, model_kwargs=None,
                                            sampling_timesteps=None):
        """Progressive native-DLPM skip sampling on the trained schedule."""
        assert self.device is not None
        self.dlpm.sample_A(shape, self.reverse_steps)
        self.dlpm.compute_Sigmas()
        ...
        for i, i_prev in time_pairs:
            t = torch.tensor([i] * shape[0], device=self.device)
            out = self.native_skip_sample(
                model, img, t, t_prev=i_prev,
                clip_denoised=clip_denoised,
                denoised_fn=denoised_fn,
                model_kwargs=model_kwargs,
            )
    ...
    def native_skip_sample_loop(self, model, shape, noise=None, clip_denoised=False,
                                denoised_fn=None, model_kwargs=None,
                                progress=False, sampling_timesteps=None):
        ...
        with torch.inference_mode():
            final = None
            for sample in self.native_skip_sample_loop_progressive(
                model, shape, noise=noise, clip_denoised=clip_denoised,
                denoised_fn=denoised_fn, model_kwargs=model_kwargs,

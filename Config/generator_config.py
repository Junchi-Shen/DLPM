...
GENERATOR_JOBS = {
    ...
    "UNet": { # <--- 名称不含数量 (假设这是 'all' 资产模型)
        ...
        "diffusion_config": { # 确保参数匹配
            ...
            "objective": 'pred_x0', "auto_normalize": False, "ddim_sampling_eta": 0.75,  # 改为0.5增加随机性（0.0是完全确定性，会导致路径相同）
        },
        "generation_params": {
    ...
    "DLPM": { # <--- DLPM模型
        ...
        "output_dir": "Path_Generator_Results_DIR",
        "model_source_folder": "all",
        "model_type_subfolder": "Diffusion_Model_DLPM",
        "use_dlpm": True,
        "use_dlpm": True,  # <-- 标记使用DLPM
        "dlpm_alpha": 1.75,  # <-- DLPM的alpha参数 (1 < alpha <= 2)
        ...
        "diffusion_config": { # 确保参数匹配 - 使用DDIM（DLIM方法）
            "seq_length": 252, "timesteps": 1000, "sampling_timesteps": 2,  # 使用100步DDIM采样（DLIM方法）
            "objective": 'pred_x0', "auto_normalize": False, "ddim_sampling_eta": 0.75,  # eta=0.5增加随机性（0.0是完全确定性，会导致路径相同）
        },
        "generation_params": {
            "runner_function": ps.run_diffusion_mega_batch,
            "num_paths_to_generate": 100, # <-- 保留默认值
    ...
    "DDPM": {
        "type": "diffusion",
        "job_name": "DDPM Path Generation (all assets model)",
        "output_filename_base": "ddpm_generated_paths",
        "output_dir": "Path_Generator_Results_DIR",
        "model_source_folder": "all",
        "model_type_subfolder": "Diffusion_Model_DDPM",
        "use_dlpm": True,   # 架构统一：DDPM = DLPM(alpha=2)
        "dlpm_alpha": 2,
        ...
        "diffusion_config": {
            "seq_length": 252, "timesteps": 1000, "sampling_timesteps": 200,
            "objective": 'pred_x0', "auto_normalize": False, "ddim_sampling_eta": 0.75,
        },
        "generation_params": {
            "runner_function": ps.run_diffusion_mega_batch,
            "num_paths_to_generate": 100,
            "generation_batch_size": 8192,
            # "sampling_timesteps": 2, 
        },
        "save_extra_outputs": False

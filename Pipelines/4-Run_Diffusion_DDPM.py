...
# 自动定位并注入模型目录
diffusion_model_dir = project_root / 'Model' / 'Diffusion_Model_DDPM'
...
try:
    ...
    import Config.Diffusion_config as DiffusionDDPMConfig
...
if __name__ == '__main__':
    ...
    # [1] 配置加载
    main_config = DiffusionDDPMConfig.main_config 
    ...
    diffusion = DLPMDiffusion1D(
        model=model,
        condition_network=condition_network,
        alpha=main_config.get('dlpm_alpha', 2),
        **main_config
    ).to(device)
    ...
    trainer = Trainer1D(
        ...
        dataset=dataset, 
        results_folder=str(pp.Model_Results_DIR / 'Diffusion_Model_DDPM' / asset_name / 'checkpoints'),
    ...
    # [6] 保存与清理
    print("\n--- 步骤 5: 产出物持久化 ---")
    model_dir = pp.Model_Results_DIR / 'Diffusion_Model_DDPM' / asset_name
    ...
    paths = {
        ...
        'report': pp.Results_DIR / "training_report" / "Diffusion_Model_DDPM" / asset_name / f"report_{timestamp_str}.md",
        'loss_curve': pp.Results_DIR / "training_report" / "Diffusion_Model_DDPM" / asset_name / f"loss_{timestamp_str}.png"
    }
